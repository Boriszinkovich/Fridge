...
- (id)initWithFrame:(CGRect)frame backGroudImageName:(NSString *)imageName subTitle:(NSString *)subTitle{
    ...
    if (self) {
       ...
       /* _subTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0.85*frame.size.height, frame.size.width, frame.size.height*0.1)];
        ...
        _subTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(30, 0.85*frame.size.height, frame.size.width - 400, frame.size.height*0.1)];
        _subTitleLabel.textAlignment = NSTextAlignmentCenter;
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
        _subTitleLabel.font = [UIFont fontWithName:@"TrebuchetMS-Bold" size:21];
        else _subTitleLabel.font = [UIFont fontWithName:@"TrebuchetMS-Bold" size:26];
        _subTitleLabel.lineBreakMode = NSLineBreakByWordWrapping;
        _subTitleLabel.numberOfLines = 0;
        _subTitleLabel.text = subTitle;
        _subTitleLabel.textColor = [UIColor whiteColor];
        ...
        NSDictionary *attributes2 = @{NSFontAttributeName:[UIFont fontWithName:@"TrebuchetMS-Bold" size:23], NSParagraphStyleAttributeName:paragraphStyle2};
     /*   cSize = [subTitle boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width-20, 40) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes2 context:nil].size;
        ...
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
            cSize = [subTitle boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width-80, 180) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes2 context:nil].size;

        else
            cSize = [subTitle boundingRectWithSize:CGSizeMake([UIScreen mainScreen].bounds.size.width-150, 130) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes2 context:nil].size;
               if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
        [_subTitleLabel setFrame:CGRectMake((frame.size.width - cSize.width)/2, frame.size.height-10-cSize.height, cSize.width, cSize.height)];
        else
             [_subTitleLabel setFrame:CGRectMake((frame.size.width - cSize.width)/2, frame.size.height-25-cSize.height, cSize.width, cSize.height)];
        [self makeBlurs];
...
-(void)updateSubViewsWithScrollOffset:(CGPoint)newOffset{
    ...
    if (self.scrollView.contentOffset.y>=-64) {
        ...
        _subTitleLabel.textAlignment = NSTextAlignmentCenter;
       _subTitleLabel.font = [UIFont fontWithName:@"TrebuchetMS-Bold" size:17];
    }
    else
    {
     //   [_subTitleLabel setFrame:CGRectMake(10, self.frame.size.height-10-cSize.height, cSize.width, cSize.height)];
            //_subTitleLabel.textAlignment = NSTextAlignmentLeft;
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
            [_subTitleLabel setFrame:CGRectMake((self.frame.size.width - cSize.width)/2, self.frame.size.height-10-cSize.height, cSize.width, cSize.height)];
        else
            [_subTitleLabel setFrame:CGRectMake((self.frame.size.width - cSize.width)/2, self.frame.size.height-25-cSize.height, cSize.width, cSize.height)];
        _subTitleLabel.lineBreakMode = NSLineBreakByWordWrapping;
        _subTitleLabel.numberOfLines = 0;
        _subTitleLabel.textAlignment = NSTextAlignmentCenter;
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
            _subTitleLabel.font = [UIFont fontWithName:@"TrebuchetMS-Bold" size:21];
        else _subTitleLabel.font = [UIFont fontWithName:@"TrebuchetMS-Bold" size:23];
    }
