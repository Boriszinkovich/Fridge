...
- (id)initWithViewControllers:(NSArray *)vCs MenuTitles:(NSArray *)titles andImagesTitles:(NSArray *)imgTitles {
    ...
    if (self) {
        ...
        {
            self.cellHeight = cellHeightForIpad;
        }
        else
        {
            self.cellHeight = cellHeightForIphone;
        }
        isPresentedFirst = NO;
...
- (void)setupMenu {
    ...
    // - Menu Button
    float buttonMenuW = 23.0;
    float buttonMenuH = 15.0;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell.imageView) {
     ...
     //    cell.imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, width, height)];
